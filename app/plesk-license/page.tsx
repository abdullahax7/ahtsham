...
const FALLBACK_PLANS: Plan[] = [
  {
    ...
    popular: true,
    orderLink: 'https://qazi.host/cart.php?a=add&pid=27',
    specs: [
  ...
  {
    name: 'Plesk (Dedicated)',
    price: 'PKR 1000.00',
    setup: 'PKR 100.00 Setup Fee',
    popular: false,
    orderLink: 'https://qazi.host/cart.php?a=add&pid=28',
    specs: [
      { label: 'Domains', value: 'Unlimited' },
      { label: 'IP Changeable', value: 'Yes' },
      { label: 'Daily Backups', value: 'Yes' },
...
const FALLBACK_HERO_TITLE = 'Plesk Panel License';
const FALLBACK_HERO_SUBTITLE = 'Plesk is a multi-faceted hosting platform that simplifies server administration by automating tasks and providing robust security features for developers and sysadmins.';

export default function PleskLicense() {
  ...
  useEffect(() => {
    let alive = true;
    fetch('/api/public/product/plesk-license')
  ...
  return (
    <main>
      ...
      <ProductSchema 
        ...
        url="https://qazi.host/plesk-license" 
        currency="PKR"
        plans={plans} 
      />
      ...
      <section className="page-hero blur-in visible">
        ...
        <div className="page-hero-content page-hero-content--split">
          ...
          <div className="hero-logo-side">
            <LicenseLogo type="plesk" glowColor="rgba(59, 130, 246, 0.4)" />
          </div>
        </div>

      </section>

      <SetupGuide type="plesk" />
      ...
      <PremiumGuarantees />
      <RelatedBlogs productSlug="plesk" title="Related Guides for Plesk" />
      <Footer />
    </main>
  );
}
