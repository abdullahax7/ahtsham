...
const FALLBACK_PLANS: Plan[] = [
  {
    ...
    popular: true,
    orderLink: 'https://qazi.host/cart.php?a=add&pid=35',
    specs: [
...
const FALLBACK_HERO_TITLE = 'Virtualizor VPS Panel License';
const FALLBACK_HERO_SUBTITLE = 'Virtualizor is a powerful web-based VPS control panel that allows server administrators to create and manage virtual private servers across multiple virtualization technologies like KVM, Xen, and OpenVZ.';

export default function VirtualizorLicense() {
  ...
  useEffect(() => {
    let alive = true;
    fetch('/api/public/product/virtualizor')
  ...
  return (
    <main>
      ...
      <ProductSchema 
        name="Virtualizor VPS Panel License" 
        url="https://qazi.host/virtualizor" 
        currency="PKR"
        plans={plans} 
      />
      ...
      <section className="page-hero blur-in visible">
        ...
        <div className="page-hero-content page-hero-content--split">
          ...
          <div className="hero-logo-side">
            <LicenseLogo type="virtualizor" glowColor="rgba(139, 92, 246, 0.4)" />
          </div>
        </div>

      </section>

      <SetupGuide type="virtualizor" />
      ...
      <PremiumGuarantees />
      <RelatedBlogs productSlug="virtualizor" title="Related Guides for Virtualizor" />
      <Footer />
    </main>
  );
}
